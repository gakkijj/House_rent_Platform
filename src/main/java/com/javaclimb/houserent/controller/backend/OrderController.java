...
@RequestMapping("/legacy/admin/order")
public class OrderController extends BaseController {
    ...
    public String allOrder(@RequestParam(value = "page",defaultValue = "1")Long pageNumber, @RequestParam(value = "size",defaultValue = "6")Long pageSize, Model model){
        ...
        for(Order order:orderPage.getRecords()){
            //todo 这个地方性能可以优化
            order.setHouse(houseService.get(order.getHouseId()));               //-todo 优化方案 ‘1’, '2','3'  id in ('1'.'2','3')
    ...
    public JsonResult cancelOrder(@RequestParam("orderId")Long orderId){
        ...
        //登录用户不是管理员，也不是该订单的租客，也不是该订单的房东，就不能取消订单
        if (!loginUserIsAdmin()&&!Objects.equals(getLoginUserId(),
                order.getCustomerUserId())&&!Objects.equals(getLoginUserId(),order.getOwnerUserId())){
    ...
    public JsonResult endOrder(@RequestParam("orderId")Long orderId){
        ...
        orderService.update(order);
        return JsonResult.success("退租已申请");
    }
    ...
    public JsonResult endOrderPass(@RequestParam("orderId")Long orderId){
        ...
        order.setTotalAmount(order.getMonthRent()*dayNum/30);
    ...
    public JsonResult endOrderReject(@RequestParam("orderId")Long orderId){
        ...
        orderService.update(order);
        return JsonResult.success("退租申请已驳回");
    }
}
